<!-- 🎈 Bubble Animasi -->
<?php for ($i = 0; $i < 30; $i++): ?>
    <div class="bubble" style="left: <?= rand(0, 100) ?>vw; width: <?= rand(40, 80) ?>px; height: <?= rand(40, 80) ?>px; animation-duration: <?= rand(12, 28) ?>s; animation-delay: <?= rand(0, 10) ?>s;"></div>
<?php endfor; ?>
    ...
    <div class="flex flex-col justify-between h-full px-4 pt-4 pb-4 relative">
        ...
        <div class="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-7 gap-4 p-4 pr-24 rounded-2xl shadow-xl overflow-y-auto bg-white/50 backdrop-blur-sm max-h-[75vh] mx-auto w-full max-w-[90vw]" style="direction: rtl;">
            <?php foreach ($huruf as $h): ?>
                <div class="card bg-white rounded-xl p-3 shadow text-center cursor-pointer hover:ring-2 hover:ring-purple-400" onclick="playSound('<?= base_url($h['suara']) ?>')">
        ...
        <img src="<?= base_url('assets/img/mini-cartoon.png') ?>" alt="Dekorasi" class="absolute bottom-10 right-4 w-20 sm:w-40 md:w-52 pointer-events-none select-none z-0" />
