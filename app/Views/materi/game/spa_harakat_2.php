...
<script>
    ...
    function renderSoal(index) {
        ...
        // daftar posisi buat tombol huruf (bisa dipake lebih dari 5)
        const posisi = [
            { top: "20%", left: "25%" },
            { top: "50%", left: "60%" },
            { top: "75%", left: "35%" },
        ...
        // ambil posisi sesuai jumlah opsi aja
        const posAcak = posisi.slice(0, opsi.length).sort(() => 0.5 - Math.random());
...
<style>
    ...
    .cloud::before { content:''; position:absolute; width:50px; height:50px; background:rgba(255,255,255,0.4); border-radius:50%; top:-25px; left:15px; }
    .cloud::after  { content:''; position:absolute; width:30px; height:30px; background:rgba(255,255,255,0.4); border-radius:50%; top:-15px; left:50px; }

    @keyframes cloud-move { 0%{ transform:translateX(-100px);} 100%{ transform:translateX(calc(100vw + 100px));} }

    .arabic-float { position:absolute; font-size:3rem; color:rgba(109,40,217,0.2); font-weight:bold; z-index:1; animation:float-up 15s linear infinite; pointer-events:none; }
    @keyframes float-up { 0%{ transform:translateY(100vh) rotate(0deg); opacity:0;} 10%{opacity:0.5;} 90%{opacity:0.5;} 100%{ transform:translateY(-100px) rotate(360deg); opacity:0;} }

    @keyframes bounce-slow { 0%,100%{ transform:translateY(0);} 50%{ transform:translateY(-15px);} }
    .animate-bounce-slow { animation:bounce-slow 3s ease-in-out infinite; }

    .confetti-particle { position:fixed; width:10px; height:10px; border-radius:50%; pointer-events:none; z-index:9999; }
    @keyframes spin { from{ transform:rotate(0);} to{ transform:rotate(360deg);} }
    .animate-spin { animation:spin 0.5s linear; }
